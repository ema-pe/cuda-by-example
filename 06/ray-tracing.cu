...
__global__ void kernel(unsigned char *image, Sphere *spheres) {
...
int main() {
    const char *filename = "ray-tracing.png";
    ...
    HANDLE_ERROR(cudaMemcpy(spheres, tmp_spheres, sizeof(Sphere) * SPHERES,
    ...
    dim3 threads(16,16);
    kernel<<<grids,threads>>>(dev_image, spheres);
